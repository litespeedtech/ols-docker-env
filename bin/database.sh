...
check_db_access(){
    docker compose exec -T mariadb su -c "mariadb -uroot --password=${MARIADB_ROOT_PASSWORD} -e 'status'" >/dev/null 2>&1
    if [ ${?} != 0 ]; then
...
check_db_exist(){
    docker compose exec -T mariadb su -c "test -e /var/lib/mariadb/${1}"
    if [ ${?} = 0 ]; then
...
db_setup(){  
    docker compose exec -T mariadb su -c 'mariadb -uroot --password=${MARIADB_ROOT_PASSWORD} \
